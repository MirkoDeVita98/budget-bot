...
from services import (
    month_key, parse_amount, looks_like_currency,
    upsert_budget, get_month_budget,
    add_rule, list_rules, delete_rule,
    compute_planned_monthly_from_rules, compute_spent_this_month,
    add_monthly_rule_named_fx, add_expense_optional_fx,
    delete_last_expense, reset_month_expenses, reset_all_user_data
)
...
async def setbudget(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return await update.message.reply_text("Couldn't parse amount. Example: /setbudget 2500")

    m = month_key()
    upsert_budget(user_id, m, amount)
    await update.message.reply_text(f"✅ Set budget for {m}: {amount:.2f} {BASE_CURRENCY}")

async def setdaily(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if len(context.args) < 2:
        return await update.message.reply_text("Usage: /setdaily <category> <amount_per_day>\nExample: /setdaily Food 15")
    category = context.args[0].strip()
    try:
        amt = parse_amount(context.args[1])
    except Exception:
        return await update.message.reply_text("Couldn't parse amount. Example: /setdaily Food 15")

    add_rule(user_id, category, f"{category} daily", "daily", amt)
    await update.message.reply_text(f"✅ Daily rule added: {category} = {amt:.2f} {BASE_CURRENCY}/day")

async def setyearly(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if len(context.args) < 2:
        return await update.message.reply_text("Usage: /setyearly <name> <amount_per_year> [category]")
    name = context.args[0].strip()
    try:
        amt = parse_amount(context.args[1])
    except Exception:
        return await update.message.reply_text("Couldn't parse amount. Example: /setyearly CarInsurance 600")

    category = context.args[2].strip() if len(context.args) >= 3 else "Yearly"
    add_rule(user_id, category, name, "yearly", amt)
    await update.message.reply_text(f"✅ Yearly rule added: {name} ({category}) = {amt:.2f} {BASE_CURRENCY}/year (→ {amt/12:.2f}/month)")
...
async def setmonthly(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) == 2:
        ...
        try:
            ...
            return await update.message.reply_text("Couldn't parse amount. Example: /setmonthly Subscriptions 35")
        add_rule(user_id, category, f"{category} monthly", "monthly", amt)
        return await update.message.reply_text(f"✅ Monthly rule added: {category} = {amt:.2f} {BASE_CURRENCY}/month")
    ...
    try:
        ...
        return await update.message.reply_text("Couldn't parse amount. Example: /setmonthly PSN 16.99 EUR Subscription")

    rule_name = " ".join(args[:amount_index]).strip() or "(no name)"

    fx_date, rate, chf_monthly = await add_monthly_rule_named_fx(user_id, rule_name, amount, currency, category)

    if currency == BASE_CURRENCY:
        await update.message.reply_text(f"✅ Monthly rule added: [{category}] {rule_name} = {chf_monthly:.2f} {BASE_CURRENCY}/month")
    else:
        await update.message.reply_text(
...
async def rules(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        lines.append(f"- ID {r['id']}: [{r['category']}] {r['name']} — {float(r['amount']):.2f} CHF / {r['period']}")
...
async def add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 3:
        return await update.message.reply_text("Usage: /add <category> <name> <amount> [currency]")
    ...
    try:
        ...
        if not looks_like_currency(currency):
            return await update.message.reply_text("Currency must be a 3-letter code (EUR, USD, ...).")
        try:
            amount = parse_amount(args[-2])
        except Exception:
            return await update.message.reply_text("Couldn't parse amount. Example: /add Travel Taxi 20 EUR")
        name = " ".join(args[1:-2]).strip() or "(no name)"

    fx_date, rate, chf_amount = await add_expense_optional_fx(user_id, category, name, amount, currency, m)

    if currency == BASE_CURRENCY:
        await update.message.reply_text(f"✅ Added: [{category}] {name} = {chf_amount:.2f} {BASE_CURRENCY}")
    else:
        await update.message.reply_text(
...
async def undo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cur == BASE_CURRENCY:
        await update.message.reply_text(f"↩️ Undid: [{row['category']}] {row['name']} = {chf:.2f} {BASE_CURRENCY}")
    else:
        await update.message.reply_text(f"↩️ Undid: [{row['category']}] {row['name']} = {orig:.2f} {cur} (was {chf:.2f} {BASE_CURRENCY})")
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if overall_budget is None:
        return await update.message.reply_text(f"📅 {m}\nNo overall budget set. Use /setbudget <amount>")
    ...
    if context.args:
        ...
        return await update.message.reply_text(
            ...
            f"Remaining: {remaining:.2f} {BASE_CURRENCY}"
        )
...
async def month(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not context.args:
        return await update.message.reply_text("Usage: /month YYYY-MM (example: /month 2025-12)")
    ...
    await update.message.reply_text(
        f"📅 {m}\n"
        f"Overall budget: {overall_budget:.2f} {BASE_CURRENCY}\n"
        f"Planned: -{planned_total:.2f} {BASE_CURRENCY}\n"
...
async def resetall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or context.args[0].lower() != "yes":
        return await update.message.reply_text("⚠️ This will DELETE ALL your data.\nRun: /resetall yes")
